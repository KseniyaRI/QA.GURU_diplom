...
Call log:
  ...
  - waiting for locator('a.MuiTypography-root.MuiTypography-inherit.MuiLink-root.MuiLink-underlineHover.css-9mh5gv')

    at /Users/kryabukhin001/Documents/QA.GURU/QA.GURU_diplom/specs/pages.spec.js:70:54
```
...
- heading "anneal alarmed shout even versus" [level=6]
...
```ts
   ...
   68 |         );
   69 |         await expect(engagementPage.clientCodeInfo).toBeVisible();
>  70 |         await expect(engagementPage.customNameField).toBeVisible();
      |                                                      ^ Error: Timed out 5000ms waiting for expect(locator).toBeVisible()
   71 |         //await expect(engagementPage.clientCodeInfo).toContainText(`Клиентский код: ${CLIENT_CODE}`);
   72 |         //await expect(engagementPage.customNameField).toContainText(engagementBuilder.customNameEng);
   73 |     });
   74 |
   75 |     test('Подписание независимости', async ({ page }) => {
   76 |         const createEngPage = new CreateEngPage(page);
   77 |
   78 |         await createEngPage.addPromise();
   79 |         await expect(createEngPage.doneEngMessage).toBeVisible();
   80 |
   81 |         await createEngPage.gotoEngagementPage();
   82 |
   83 |     });
   84 |
   85 |     test('Проверка пагинации на странице Engagements', async ({ page }) => {
   86 |         const engagementsPage = new EngagementsPage(page);
   87 |
   88 |         await engagementsPage.usePagination();
   89 |
   90 |         await expect(engagementsPage.firstPaginationIcon).toContainText('1–50 из 411');
   91 |     });
   92 |
   93 |     test('Переход на страницу списка запросов (логов) Engagement', async ({ page }) => {
   94 |         const engagementsPage = new EngagementsPage(page);
   95 |
   96 |         await engagementsPage.gotoEngagementLogsPage();
   97 |
   98 |         await expect(engagementsPage.dropDownEngLogs).toBeVisible(); 
   99 |     });
  100 | });
```