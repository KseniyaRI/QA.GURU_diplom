...
Call log:
  - waiting for getByRole('combobox', { name: '2025' })

    at CreateEngPage.createEngagement (/Users/kryabukhin001/Documents/QA.GURU/QA.GURU_diplom/sources/pages/createEngPage.js:36:42)
...
- button "Закрыть"
- paragraph
- listbox "Период проверки":
  - option "3 мес" [selected]
  - option "6 мес"
  - option "9 мес"
  - option "12 мес"
...
- textbox "Название клиента / Audit / IFRS / 3 мес / 2023": Happy friday / Согласованные / УП Группы / 3 мес
- paragraph
...
```ts
  ...
  27 |         await this.page.getByRole('option', { name: 'Happy friday' }).click();
  28 |         await this.departmentField.click();
  29 |         await this.page.getByRole('option', { name: departmentName }).click();
  30 |         await this.typeField.click();
  31 |         await this.page.getByRole('option', { name: type }).click();
  32 |         await this.reportingStandardField.click();
  33 |         await this.page.getByRole('option', { name: reportingStandard }).click();
  34 |         await this.verificationPeriodsField.click({ force: true });
  35 |         await this.page.getByRole('option', { name: verificationPeriods }).click();
> 36 |         await this.verificationYearField.click({ force: true });
     |                                          ^ Error: locator.click: Test timeout of 60000ms exceeded.
  37 |         await this.page.getByRole('option', { name: verificationYear }).click();
