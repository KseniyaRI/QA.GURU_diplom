...
```
Error: expect.toContainText: Error: strict mode violation: locator('div').filter({ has: getByRole('combobox', { name: 'Название клиента' }) }).locator('..') resolved to 13 elements:
    1) <body>…</body> aka locator('body')
    2) <div id="root">…</div> aka locator('#root')
    3) <div class="MuiBox-root css-1k9sv55">…</div> aka locator('.MuiBox-root').first()
    4) <div class="MuiBox-root css-apy4gz">…</div> aka locator('div').filter({ hasText: 'Создание проектаЗаполните данные о проекте. Введенные данные сформируют структур' }).nth(2)
    5) <div class="MuiBox-root css-15mrgmy">…</div> aka locator('div').filter({ hasText: 'Создание проектаЗаполните данные о проекте. Введенные данные сформируют структур' }).nth(3)
    6) <form>…</form> aka locator('form')
    7) <div class="MuiBox-root css-a1ap8x">…</div> aka locator('form div').filter({ hasText: 'Заполните данные о проекте. Введенные данные сформируют структурированное назван' }).first()
    8) <div class="MuiBox-root css-13dpkcn">…</div> aka locator('form div').filter({ hasText: 'Заполните данные о проекте. Введенные данные сформируют структурированное назван' }).nth(1)
    9) <div class="MuiGrid-root MuiGrid-container css-1ukm4vn">…</div> aka getByText('Головной клиент (клиентский код)*Обязательно для заполненияОтдел*Обязательно для')
    10) <div class="MuiGrid-root MuiGrid-item MuiGrid-grid-xs-2 css-15j76c0">…</div> aka locator('.MuiGrid-root > div').first()
    ...

Call log:
  - expect.toContainText with timeout 5000ms
  - waiting for locator('div').filter({ has: getByRole('combobox', { name: 'Название клиента' }) }).locator('..')

    at /Users/kryabukhin001/Documents/QA.GURU/QA.GURU_diplom/specs/pages.spec.js:29:52
```
...
```ts
  ...
  28 |
> 29 |         expect(createEngPage.clientFieldContainer).toContainText('Обязательно для заполнения');
     |                                                    ^ Error: expect.toContainText: Error: strict mode violation: locator('div').filter({ has: getByRole('combobox', { name: 'Название клиента' }) }).locator('..') resolved to 13 elements:
  30 |
  31 |     });
  32 |
