...
- Name: Создание Engagement и работа с ним >> Проверка валидации при попыытке создать Engagement с незаполненными полями
...
Locator: getByRole('button', { name: 'Создать' })
Expected: visible
...
Call log:
  ...
  - waiting for getByRole('button', { name: 'Создать' })
    ...
    at /Users/kryabukhin001/Documents/QA.GURU/QA.GURU_diplom/specs/pages.spec.js:61:44
```
...
```ts
  ...
  54 |
  55 |         await expect(engagementsPage.dropDownEngLogs).toBeVisible(); 
  56 |     });*/
  57 |
  58 |     test('Проверка валидации при попыытке создать Engagement с незаполненными полями', async ({ page }) => {
  59 |         const createEngPage = new CreateEngPage(page);
  60 |
> 61 |         expect(createEngPage.createButton).toBeVisible();
     |                                            ^ Error: expect(locator).toBeVisible()
  62 |
  63 |     });
  64 | });
