...
Locator: locator('div:has-text("Название клиента") + div p.MuiFormHelperText-root.Mui-error:has-text("Обязательно для заполнения")')
Expected: visible
Received: <element(s) not found>
Call log:
  - expect.toBeVisible with timeout 5000ms
  - waiting for locator('div:has-text("Название клиента") + div p.MuiFormHelperText-root.Mui-error:has-text("Обязательно для заполнения")')
