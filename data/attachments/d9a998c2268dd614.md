...
Expected: "Tatiana Rusanova"
Received: "TRTatiana Rusanova"
    at /Users/kryabukhin001/Documents/QA.GURU/QA.GURU_diplom/specs/pages.spec.js:68:36
```
...
```ts
  ...
  62 |
  63 |         // Получаем текст из обоих элементов
  64 |         const usernameButtonText = await engagementsPage.usernameButton.textContent();
  65 |         const dropDownUserDataText = await engagementsPage.dropDownUserData.textContent();
  66 |
  67 |         // Сравниваем тексты
> 68 |         expect(usernameButtonText).toBe(dropDownUserDataText);
     |                                    ^ Error: expect(received).toBe(expected) // Object.is equality
  69 |     });
  70 | });
  71 |
