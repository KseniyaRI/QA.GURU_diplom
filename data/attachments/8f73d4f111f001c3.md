...
Expected: "4ed1fcb3-96ad-4a2f-be79-2fda08f90544"
Received: "3b891e10-ec8a-4eb0-9e1f-f3a398ec1dca"
    at /Users/kryabukhin001/Documents/QA.GURU/QA.GURU_diplom/specs/api.spec.js:113:49
```
...
```ts
   ...
   97 |         },
   98 |         body: {departmentId: "3b891e10-ec8a-4eb0-9e1f-f3a398ec1dca"}
   99 |       });
  100 |       
  101 |       expect(updateEngResponse.status()).toBe(200);
  102 |
  103 |       // запрашиваем обновленный Engagement
  104 |       const updatedEngResponse = await request.get(`${BASE_URL}/api/engagements/${firstEngId}`, {
  105 |         headers: {
  ...
  109 |       });
  110 |
  111 |       const updatedEngBody = await updatedEngResponse.json();
  112 |       // проверяем, что поля Engagement обновились
> 113 |       expect(updatedEngBody.data.department.id).toBe(engagment.departmentId);
      |                                                 ^ Error: expect(received).toBe(expected) // Object.is equality
  114 |
  115 |     //test('Получение истории действий c Engagment', async ({ request }) => {
  116 |     });
  117 | });
  118 |
  119 |
